...
import {
  AddCategoryProps,
} from "@components/budgets/AddBudgetCategory/types";
...
import { useCreateBudgetCategoryMutation, useListBudgetCategoriesQuery } from "@services/budget";

const AddBudgetCategory = ({ closeModal, edit, id }: AddCategoryProps) => {
  ...
  const [createBudgetCategory] = useCreateBudgetCategoryMutation();
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (edit) {
      ...
      res = await createBudgetCategory({ data, accessToken })
        .unwrap()
        .catch((resp) => {
          setAlert({
  ...
  return (
    <div className="space-y-10">
      <div className="">
        <h2 className="title">Add budget category</h2>
      </div>
